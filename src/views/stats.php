...
<ul class="nav nav-pills mb-5">
    <?php for ($i = $topSeason; $i > 0; $i--): ?>
        <li class="nav-item">
            <a class="nav-link <?= $i === $season ? 'active' : '' ?>" href="/stats/<?= $i ?>">Season <?= $i ?></a>
        </li>
    <?php endfor; ?>
</ul>
<h2>Season <?= $season ?></h2>
<div class="row">
    ...
    foreach ($playerStats as $playerStat): $percent = round($playerStat['score']['total'] * 100 / $totalPossibleMatches); ?>
        <div class="card float-left mb-3 mr-3" style="width:16rem;">
            ...
            <ul class="list-group list-group-flush">
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    Current streak<span class="badge badge-<?= $playerStat['streak']['badge'] ?>">
                        ...
                        <?= $playerStat['streak']['badge'] === 'success' ? 'WIN' : 'LOSS' ?><?= $playerStat['streak']['matches'] > 1
                            ? ($playerStat['streak']['badge'] === 'success' ? 'S' : 'ES')
                            : '' ?>
                    </span>
                </li>
                <li class="list-group-item d-flex justify-content-between align-items-center">
